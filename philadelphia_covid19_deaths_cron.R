...
readr::write_csv(deaths_step1a, stringr::str_c(Sys.getenv("COVID19PHILLY_DIR"),"deaths_by_zipcode/", 
                           "covid_deaths_by_zip_", 
                           paste0(lubridate::today()), 
                           ".csv.gz"))

# Philadelphia COVID-19 daily deaths data by sex and age
deaths_step1b <- readr::read_csv(
  ...
  col_types = readr::cols(count = readr::col_integer(), 
                          etl_timestamp = readr::col_character()))
readr::write_csv(deaths_step1b, stringr::str_c(Sys.getenv("COVID19PHILLY_DIR"),"deaths_by_sex_age/", 
                                        "covid_deaths_by_sex_age_", 
                                        paste0(lubridate::today()), 
                                        ".csv.gz"))

# Philadelphia COVID-19 daily deaths data by date
deaths_step1c <- readr::read_csv(
  ...
  col_types = readr::cols(clinical_date_of_death = readr::col_character(),
                          count = readr::col_integer(), 
                          etl_timestamp = readr::col_character()))
readr::write_csv(deaths_step1c, stringr::str_c(Sys.getenv("COVID19PHILLY_DIR"),"deaths_by_date/", 
                                        "covid_deaths_by_date_", 
                                        paste0(lubridate::today()), 
                                        ".csv.gz"))

# Philadelphia COVID-19 daily deaths data by race
deaths_step1d <- readr::read_csv(
  ...
  col_types = readr::cols(racial_identity = readr::col_character(),
                          count = readr::col_integer(), 
                          etl_timestamp = readr::col_character()))
readr::write_csv(deaths_step1d, stringr::str_c(Sys.getenv("COVID19PHILLY_DIR"),"deaths_by_race/", 
